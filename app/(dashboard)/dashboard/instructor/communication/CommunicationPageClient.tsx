...
import { getCommunicationCourseOptions } from '@/lib/instructor-communication-data'

export default function CommunicationPageClient() {
  const options = getCommunicationCourseOptions().filter((o) => o.value !== '')
  return (
    <div className="rounded-xl border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 p-6">
      <p className="mb-4 text-sm font-medium text-gray-700 dark:text-gray-300">
        Select a course for announcements, Q&A, and live sessions
      </p>
      <ul className="space-y-2">
        {options.map((c) => (
          <li key={c.value}>
            <Link
              href={`/dashboard/instructor/communication?course=${c.value}`}
